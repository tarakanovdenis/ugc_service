...
video_players.forEach(video_player => {
  ...
  function sendPauseEvent(pause_at) {
  ...
  mainVideo.addEventListener("pause", () => {
    sendPauseEvent(mainVideo.currentTime);
    pauseVideo();
  });
  ...
  function sendQualityChangeEvent(quality_before, quality_after, changed_at) {
  ...
  var quality_before
  quality_li.forEach((event) => {
    if (event.getAttribute('class') === 'active') {
      let active_quality = event.getAttribute('data-quality');
      quality_before = active_quality
    }
    event.addEventListener('click', (e) => {
      let quality_after = event.getAttribute('data-quality');
      if (quality_before != quality_after) {
        sendQualityChangeEvent(quality_before, quality_after, mainVideo.currentTime)
      }
